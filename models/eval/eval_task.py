...
class EvalTask(Eval):
    ...
    def evaluate(cls, env, model, r_idx, resnet, traj_data, args, lock, successes, failures, results):
        ...
        while not done:
            ...
            # if not args.use_action_feasibility_oracle or t_success or (action != 'MoveAhead_25'):
            #     completed_valid_action = True
            #     if args.debug:
            #         print(t_success, action)
            # # Mask out the unfeasible action, let the model pick the next best action
            # else:
            #     if args.debug:
            #         print(t_success, 'unfeasible move ahead', action)
        
            if args.debug:
                    print(t_success, action)
            if not t_success and action == 'MoveAhead_25':
