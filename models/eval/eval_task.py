    ...
    def evaluate(cls, env, model, r_idx, resnet, traj_data, args, lock, successes, failures, results):
        ...
        while not done:
            ...
            while not completed_valid_action:
                ...
                mask = None
                if model.has_interaction(action) and mask is None:
                ...
                assert not (model.has_interaction(action) and mask is None)
                ...
                if not args.use_action_feasibility_oracle or t_success or (args.navigation_actions_only and action != 'MoveAhead_25'):
                    completed_valid_action = True
                    if args.debug:
                        print('move on', t_success, action)
                # Mask out the unfeasible action, let the model pick the next best action
                    ...
                    # print(m_out['out_action_low'][0][0])
                    if args.debug:
                        print('unfeasible action', t_success, action)
            
            # If while loop did not break executes else statement
            else:
                if not t_success:
                    fails += 1
                ...
                prev_action = action
                continue
            # Otherwise we break the larger loop and we are done with this trial.
            break
